...
import { FileWarning, Loader2, CheckCircle2, ShieldAlert, Paperclip } from "lucide-react";
...
export default function ReportSection() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!consent) {
      setErrMsg("Vous devez accepter la confidentialité et l'usage des données.");
      return;
    }
      ...
      <div className="relative mx-auto max-w-7xl px-6 md:px-10">
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-3 text-white/80">
            Décrivez brièvement ce qui s’est passé. Votre signalement contribue à améliorer la
            détection et à protéger d’autres personnes. Votre email est facultatif.
          </p>
        </div>

        {/* Form card */}
        <div className="mx-auto mt-8 w-full max-w-3xl rounded-2xl bg-white/5 p-5 ring-1 ring-white/10 backdrop-blur">
          <form onSubmit={handleSubmit} className="grid gap-4">
                ...
                {(["phone", "email", "url"] as ReportType[]).map((t) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm transition ${
                      type === t ? "bg-primary text-black font-semibold" : "bg-white/10 text-white/90 hover:bg-white/15"
                    }`}
                  >
                    {t === "phone" ? "Numéro de téléphone" : t === "email" ? "Email" : "URL"}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <label className="mb-1 block text-sm text-white/80">
                {type === "phone" ? "Numéro" : type === "email" ? "Adresse email" : "Lien (URL)"} <span className="text-red-300">*</span>
              </label>
              <input
                required
                value={value}
                ...
                placeholder={
                  type === "phone" ? "+1 514 555 0000" : type === "email" ? "exemple@domaine.com" : "https://exemple.com"
                }
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="mb-1 block text-sm text-white/80">Ville (optionnel)</label>
                <input
                  value={city}
              ...
              <div>
                <label className="mb-1 block text-sm text-white/80">Email de contact (optionnel)</label>
                <input
                  type="email"
                  value={reporterEmail}
            ...
            {/* Evidence URL (demo) */}
            <div>
              <label className="mb-1 block text-sm text-white/80">Preuve (lien image / capture — optionnel)</label>
              ...
              <p className="mt-1 text-xs text-white/50">
                (Démo) Collez un lien public. Pour un vrai upload sécurisé (S3), je peux l’ajouter ensuite.
              </p>
            </div>

            {/* Consent */}
            <label className="mt-1 inline-flex items-start gap-2 text-sm text-white/80">
              <input
                type="checkbox"
                checked={consent}
                onChange={(e) => setConsent(e.target.checked)}
                className="mt-1 h-4 w-4 rounded-sm border-white/20 bg-transparent"
              />
              <span>
                J’accepte que les informations fournies soient utilisées pour la détection et la prévention de la fraude.
                Mon email (si fourni) ne sera pas rendu public.
              </span>
            </label>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="rounded-full bg-white/10 px-5 py-3 text-sm ring-1 ring-white/15 transition hover:bg-white/15"
              >
                Réinitialiser
              </button>
            </div>
            ...
            {/* Messages */}
            {okMsg && (
              <div className="mt-3 inline-flex items-center gap-2 rounded-xl bg-emerald-500/15 px-4 py-3 text-sm text-emerald-200 ring-1 ring-emerald-400/30">
            ...
            {errMsg && (
              <div className="mt-3 inline-flex items-center gap-2 rounded-xl bg-red-500/15 px-4 py-3 text-sm text-red-200 ring-1 ring-red-400/30">
        ...
        <p className="mx-auto mt-4 max-w-3xl text-center text-xs text-white/60">
          Nous n’exposons jamais vos informations personnelles. Les identifiants (numéro/email/URL) alimentent une base
          publique avec niveaux de confiance et modération.
        </p>
      </div>
    </section>
  );
